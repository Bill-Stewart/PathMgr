...
begin
  ...
  for I := 0 to Length(Dirs) - 1 do
    if StrHasNonWhitespace(Dirs[I]) then
      JoinString(ResultStr, NormalizePath(Dirs[I]), NEWLINE);
end;
...
var
  ...
  I: LongInt;
begin
  if not IsValidPath(DirName) then
...
begin
  ...
  if PathFindType <> FoundInUnexpandedPath then
    exit(ERROR_PATH_NOT_FOUND);
  InitRegKeyAndPath(PathType, RegRoot, SubKeyName);
  result := RegQueryStringValue(RegRoot, SubKeyName, REG_VALUE_NAME, Path);
  if result <> ERROR_SUCCESS then
